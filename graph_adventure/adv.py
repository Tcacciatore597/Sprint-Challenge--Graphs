...
#             print(vertex)
#             vis.add(vertex)
#             direct = roomGraph[vertex][1]
#             for neighbor in direct:
#                 s.push(direct[neighbor])
# dfs(player)
# def move_and_check(player):
#     valid_path = False
...
#         dir_pick = random.choice(exits)
# # if you have already explored that direction, pick a different direction.
#         player.travel(dir_pick)
...
#     if v not in visited:
#         visited.add(v)
#         for connection in roomGraph:
#             s.push(connection)
        
# add the id to visited rooms
# # loop through connected room ids and add them to the stack.
#if there are no exits that you have not already visited. then backtrack and find the first unexplored exit and repeat.

#traversal path is a list of directions
traversalPath = []

# Traversing entire map using a BFT
q = Queue()
